...
unsigned char AT89S51_Program_write(void)
{
	...
	for (program_cnt = 0; program_cnt < hex_size1; program_cnt ++) {
		AT89S51_Write_Byte(program_cnt + 0x0000, hex_file1[program_cnt]);
			delay_ms(1);
	}
	for (program_cnt = 0; program_cnt < hex_size2; program_cnt ++) {
		AT89S51_Write_Byte(program_cnt + 0x0013, hex_file2[program_cnt]);
			delay_ms(1);
	}
	for (program_cnt = 0; program_cnt < hex_size3; program_cnt ++) {
		AT89S51_Write_Byte(program_cnt + 0x0030, hex_file3[program_cnt]);
			delay_ms(1);
	}
...
unsigned char AT89S51_Program_read(void)
{
	...
	for (program_cnt = 0; program_cnt < hex_size1; program_cnt ++) {
		...
		if(spr_r_buf != hex_file1[program_cnt])
		{
			AT8051_RST = 0;
				delay_ms(100);
			AT8051_MOSI = 1;
	...
	for (program_cnt = 0; program_cnt < hex_size2; program_cnt ++) {
		spr_r_buf = AT89S51_Read_Byte(program_cnt + 0x0013);
			delay_ms(1);
		
		// check if the read data is equal to hexfile
		if(spr_r_buf != hex_file2[program_cnt])
		{
			AT8051_RST = 0;
				delay_ms(100);
			AT8051_MOSI = 1;
	...
	for (program_cnt = 0; program_cnt < hex_size3; program_cnt ++) {
		...
		// check if the read data is equal to hexfile
		if(spr_r_buf != hex_file3[program_cnt])
		{
			AT8051_RST = 0;
				delay_ms(100);
			AT8051_MOSI = 1;
